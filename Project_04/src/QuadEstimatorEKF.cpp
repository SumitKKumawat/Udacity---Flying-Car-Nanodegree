...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  // Returning the partial derivative of the Rbg rotation matrix with respect to yaw. We call this RbgPrime.
  float cos_theta_angl = cos(pitch);

  float sin_theta_angl = sin(pitch);
  
  float cos_phy_angl = cos(roll);

  float sin_phy_angl = sin(roll);
  
  float sin_sy_angl = sin(yaw);

  float cos_sy_angl = cos(yaw);
  

  RbgPrime(0,0) = - cos_theta_angl * sin_sy_angl;

  RbgPrime(0,1) = - sin_phy_angl  * sin_theta_angl * sin_sy_angl - cos_theta_angl * cos_sy_angl;

  RbgPrime(0,2) = - cos_phy_angl  * sin_theta_angl * sin_sy_angl + sin_phy_angl   * cos_sy_angl;
  
  RbgPrime(1,0) = cos_theta_angl * cos_sy_angl;

  RbgPrime(1,1) = sin_phy_angl  * sin_theta_angl * cos_sy_angl - cos_phy_angl * sin_sy_angl;

  RbgPrime(1,2) = cos_phy_angl  * sin_theta_angl * cos_sy_angl + sin_phy_angl * sin_sy_angl;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  // Predicting the current covariance forward by dt using the current accelerations and body rates as input.
  gPrime(0,3) = dt;
